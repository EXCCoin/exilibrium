...
const Page1 = StandardPage(
  "lifecycle-01",
  <T
    id="tutorial.ticketLifecycle.page1"
    m={`Purchasing a ticket for PoS is quite simple but what happens to it after you buy it? A ticket will go through a few stages in its lifetime:
`}
  />
);
...
const Page2 = StandardPage(
  "lifecycle-02",
  <T
    id="tutorial.ticketLifecycle.page2"
    m={`When a Ticket is purchased, the total cost is its Ticket Fee + Ticket Price. The price is dynamic in order to yield a ticket pool of approximately 40,960 tickets.
...
const Page3 = StandardPage(
  "lifecycle-03",
  <T
    id="tutorial.ticketLifecycle.page3"
    m={`Two things might happen next:
...
const Page4 = StandardPage(
  "lifecycle-04",
  <T
    id="tutorial.ticketLifecycle.page4"
...
const Page5 = StandardPage(
  "lifecycle-05",
  <T
    id="tutorial.ticketLifecycle.page5"
    m={`You must be connected to the network to cast a vote, which is why many people use stakepools. If your ticket is called and it misses the vote, you receive a refund on the original Ticket Price.
...
const Page6 = StandardPage(
  "lifecycle-06",
  <T
    id="tutorial.ticketLifecycle.page6"
    m={`You should now understand a bit better the lifecycle of a ticket, so start staking your EXCC and participate on the community!
`}
  />
);
