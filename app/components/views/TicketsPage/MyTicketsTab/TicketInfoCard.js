...
const TicketInfoCard = ({ ticket, onClick, expanded }) => {
  ...
  return (
    <TicketCard {...{ className, status: ticket.status }} onClick={() => onClick(ticket)}>
      <div className="ticket-info-expanded-indicator" />
      <div className="ticket-info-price">
        <Balance amount={ticket.ticketPrice} />
      </div>
      {returnTipText ? (
        <Tooltip tipWidth={300} text={returnTipText} className="tooltip-pre-line">
          <div className="ticket-reward">
            <Balance amount={ticket.ticketReward} noSmallAmount />
          </div>
        </Tooltip>
      ) : null}
