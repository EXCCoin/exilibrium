...
const updateSavedConfig = (newPoolInfo, poolHost, apiKey, accountNum) => (dispatch, getState) => {
  const currentStakePoolConfig = sel.currentStakePoolConfig(getState());
  const stakePoolConfigs = currentStakePoolConfig.map(
    config =>
      ...
      setStakePoolAddress(poolHost, apiKey, publicKey)
        .then(response => {
          if (response.data.status === "success") {
            ...
            dispatch({
              error: "shouldn't be here set address:",
